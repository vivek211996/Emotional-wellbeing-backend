...
app.post("/", async (req, res) => {
    ...
    if(req.body.SubmitType === "Login"){
      ...
      try {
        ...
        let dataUser = await db.collection("Login-Emotional health management").find({Username:req.body.name }).toArray();
        await client.close();
...
bcrypt.compare(req.body.password, dataUser[0].pwd, function(err, result) {
      ...
      try {
        ...
        let client = await mongodb.connect(url);
        let db = client.db("CRM");
        let data = await db.collection("Login-Emotional health management").insertOne({Username : req.body.name, pwd: req.body.password,email:req.body.email});
        await client.close();
...
console.log(error)
    ...
    app.get("/checkinterested",async function(req,res){
  ...
  app.get("/username",async function(req,res){
    ...
    try {
        ...
        console.log(url)
       let db = client.db("CRM")
        let data = await db.collection("Login-Emotional health management").find().toArray();
        await client.close(data);
        res.json(data)
        console.log(data)
  ...
  app.get("/assessment1",async function(req,res){
    ...
    try {
        ...
        console.log(url)
       let db = client.db("CRM")
        let data = await db.collection("Assessment1").find().toArray();
        await client.close(data);
        res.json(data)
        console.log(data)
